...
class WebGPU {
    ...
    static createRenderPassDescriptor() {
        const description = {
            colorAttachments: [{
                    ...
                    clearValue: WebGPU._config.clearColor,
                    loadOp: 'clear',
                    storeOp: 'store'
                }],
        };
        if (WebGPU._config.antialias == 4) {
            description.colorAttachments[0].view = WebGPU.renderTarget.createView();
            description.colorAttachments[0].resolveTarget = WebGPU.ctx.getCurrentTexture().createView();
        }
        if (WebGPU._config.depth)
            return Object.assign(Object.assign({}, description), { depthStencilAttachment: {
                    view: WebGPU.depthTexture.createView(),
                    depthClearValue: 1.0,
                    depthLoadOp: 'clear',
                    depthStoreOp: "store",
                } });
    ...
    writeImageOnTexture(image, texture) {
    ...
    writeBuffer(buffer, data, type, offset = 0) {
    ...
    setRenderPassDescriptorView(renderPassDescriptor) {
        if (WebGPU._config.antialias === 1) {
            renderPassDescriptor.colorAttachments[0].view = WebGPU.ctx.getCurrentTexture().createView();
            return renderPassDescriptor;
        }
        renderPassDescriptor.colorAttachments[0].view = WebGPU.renderTarget.createView();
        renderPassDescriptor.colorAttachments[0].resolveTarget = WebGPU.ctx.getCurrentTexture().createView();
        return renderPassDescriptor;
    }
}
WebGPU._config = {
